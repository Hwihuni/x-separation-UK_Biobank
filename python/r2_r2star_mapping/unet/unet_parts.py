...
class Up1_33(nn.Module):
    ...
    def forward(self, x1, x2):
        x1 = self.up(x1)
        x = torch.cat([x2[:,:,15:79,15:79], x1], dim=1)
        return self.conv(x)
...
class OutConv(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv = nn.Sequential( nn.Conv2d(in_channels, out_channels, kernel_size=1), nn.ReLU(inplace=True))
